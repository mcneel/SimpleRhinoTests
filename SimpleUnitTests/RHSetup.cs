...
namespace Rhino.Test
{
    public abstract class RhinoTestFixture
    {
        ...
        {
            RhinoGrasshopperSingleton.Instance.Dispose();
        }
    }

    internal class RHSetup : IDisposable
    {
        private object _Doc { get; set; }
        public RhinoGrasshopperSingleton GH => RhinoGrasshopperSingleton.Instance;
    ...
    public class RhinoGrasshopperSingleton : IDisposable
    {
        private static bool _isResolverInitialized = false;
        private static RhinoGrasshopperSingleton _instance = null;
        public static RhinoGrasshopperSingleton Instance
        {
            get
            {
                if (null == _instance)
                    _instance = new RhinoGrasshopperSingleton();
                return _instance;
            }
        }
        static RhinoGrasshopperSingleton()
        {
            // This MUST be included in a static constructor to ensure that no Rhino DLLs
            // are loaded before the resolver is set up. Avoid creating other static functions
            // and members which may reference Rhino assemblies, as that may cause those
        ...
        public RhinoGrasshopperSingleton()
        {
            InitializeCore();
        }
